...
El bloc `splash` permet que aparega un missatge en pantalla perquè el jugador el llija i després el descarte amb una pulsació de botó.
...
Les variables permeten que el codi es pugui canviar fàcilment segons siga necessari per al teu joc. En python, les variables són declarades amb el nom de la variable seguit d'un signe igual i el valor que vols assignar-li.
...
Les variables es poden reassignar en qualsevol moment tantes vegades com siga necessari. Per exemple, el següent codi reassigna la variable `missatge` amb el valor `"Hola Món!"` i després amb el valor `"Adeu Món!"`:
